        ...
        <div className={styles.dashboardLayout}>
          <section className={styles.mainPanel}>
            ...
            <div className={styles.card} style={{ gap: '1rem' }}>
              ...
              <p className={styles.partnerDescription} style={{ fontSize: '0.95rem', lineHeight: '1.6', WebkitLineClamp: 'none', lineClamp: 'none' }}>
                {partner.description || 'Este parceiro ainda não forneceu uma descrição detalhada.'}
              </p>
              
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem', fontSize: '0.9rem', color: 'var(--color-text-muted, #9ca3af)', borderTop: '1px solid var(--color-border, rgba(255,255,255,0.08))', paddingTop: '1rem' }}>
                <div>
                  <p style={{ marginBottom: '0.5rem' }}><strong style={{ color: 'var(--color-text, #fff)' }}>📍 Endereço:</strong> {partner.address}</p>
                  <p style={{ marginBottom: '0.5rem' }}><strong style={{ color: 'var(--color-text, #fff)' }}>🌆 Cidade:</strong> {partner.city ? `${partner.city} - ${partner.state}` : 'Não informada'}</p>
                </div>
                <div>
                  <p style={{ marginBottom: '0.5rem' }}><strong style={{ color: 'var(--color-text, #fff)' }}>📞 Contato:</strong> {partner.phone}</p>
                  <p style={{ marginBottom: '0.5rem' }}><strong style={{ color: 'var(--color-text, #fff)' }}>🚗 Região Atendimento:</strong> {partner.deliveryRegion || 'Local'}</p>
                </div>
              </div>
            </div>
                  ...
                  {products.map((product) => (
                    <div key={product.id} className={styles.card} style={{ padding: '1.25rem', gap: '0.5rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <h3 className={styles.partnerName} style={{ fontSize: '1.1rem', fontWeight: '700' }}>{product.name}</h3>
                        {product.hasGluten ? (
                      ...
                      <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted, #9ca3af)' }}>Marca: {product.brand}</p>
                      <p style={{ fontSize: '0.8rem', color: 'var(--color-text-muted, #9ca3af)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          ...
          <aside className={styles.sidePanel}>
            <div className={styles.card} style={{ gap: '1rem' }}>
              ...
              <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
                ...
                <h3 className={styles.partnerName} style={{ marginTop: '1rem', fontSize: '1.1rem', fontWeight: '700' }}>Parceiro Homologado</h3>
                <p style={{ fontSize: '0.8rem', color: 'var(--color-text-muted, #9ca3af)', marginTop: '0.5rem', lineHeight: '1.4' }}>
